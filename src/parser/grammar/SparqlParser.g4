...
// SPARQL 1.2 Parser grammar derived from W3C BNF: https://www.w3.org/TR/sparql12-query/ Designed
// for safe import/embedding within the Fifth language parser. Top-level rule name is 'queryUnit' to
// avoid conflicts with other grammars.
...
// [8] SelectClause
selectClause: SELECT ( DISTINCT | REDUCED)? ( ( varOrProjection)+ | STAR);

varOrProjection: var | LPAREN expression AS var RPAREN;
...
// [40] DataBlockValue
dataBlockValue: iri | rdfLiteral | numericLiteral | booleanLiteral | tripleTerm | UNDEF;
...
propertyListNotEmpty:
    verb objectList (reifier | annotation)* (SEMI ( verb objectList ( reifier | annotation)*)?)*
;
...
triplesSameSubjectPath:
    ...
    | tripleTerm
; // allow standalone triple term in BGPs
...
// [69] PathPrimary
pathPrimary: iri | A | NOT_SIGN pathNegatedPropertySet | LPAREN path RPAREN;
...
pathOneInPropertySet: iri | A | CARET ( iri | A);
...
// Allow optional embedded reifier before closing >> per SPARQL 1.2 tests:
// Syntax: << S P O ~ R >> or << S P O >>
// R may be iri, blank node, variable, or nested triple term
tripleTerm:
    LT2 (
        dataValueTerm dataValueTerm dataValueTerm
        | LPAREN dataValueTerm dataValueTerm dataValueTerm RPAREN
    ) (TILDE ( iri | blankNode | var | tripleTerm))? GT2
;
...
// [104] Aggregate
aggregate:
    COUNT LPAREN DISTINCT? (STAR | expression) RPAREN
