...
- [ ] [T019] [US1] Create `.github/workflows/release.yml` with trigger configuration (tag push pattern `v[0-9]+.[0-9]+.[0-9]+*` and workflow_dispatch with dry_run input) per contract in `contracts/github-actions-workflow.md`
- [ ] [T020] [US1] Add concurrency control to `.github/workflows/release.yml` with group `release-${{ github.ref }}` and `cancel-in-progress: true` (FR-032) per contract in `contracts/github-actions-workflow.md` - depends on T019
- [ ] [T021] [US1] Define build matrix in `.github/workflows/release.yml` with all 12 platform/framework combinations (6 platforms × 2 frameworks), set `fail-fast: false` for diagnostics per contract in `contracts/github-actions-workflow.md` - depends on T019
- [ ] [T022] [US1] Implement build job steps in `.github/workflows/release.yml`: checkout (fetch-depth: 0), setup-dotnet (global.json), setup-java (17, temurin), check framework SDK availability with graceful degradation (FR-033), extract version, build package, verify package, smoke test, upload artifact - depends on T021, T010, T011, T015, T016, T005
- [ ] [T023] [US1] Implement publish job in `.github/workflows/release.yml` with `needs: [build]` and `if: ${{ success() }}` to enforce all-or-nothing policy (FR-030), download artifacts, check framework coverage, check package sizes with soft limit warnings (FR-031), generate checksums, generate release notes with framework/size warnings, create GitHub Release - depends on T022, T012
- [ ] [T024] [US1] Add pre-release detection logic to `.github/workflows/release.yml` publish job (check if version contains suffix like -alpha, -beta, -rc) per contract in `contracts/github-actions-workflow.md` - depends on T023
- [ ] [T025] [US1] Test workflow locally using `act` or validate YAML syntax with GitHub Actions CLI - depends on T023
